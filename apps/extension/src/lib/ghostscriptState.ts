...
export async function ensureConversationState(threadId: string) {
  ...
  if (state.conversationsByThreadId[threadId]) {
    return state.conversationsByThreadId[threadId];
  }
...
export async function readConversationState(threadId: string) {
  const state = await readGhostscriptState();
  return state.conversationsByThreadId[threadId] ?? createEmptyConversationState(threadId);
}
...
export async function updateConversationState(
  ...
  const state = await readGhostscriptState();
  const currentConversation = state.conversationsByThreadId[threadId] ?? createEmptyConversationState(threadId);
